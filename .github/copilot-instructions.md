...
`docker-mcp` is a Python MCP (Model Context Protocol) server that exposes the Docker SDK for Python — plus selected docker CLI features (Compose, Context, Buildx, Scout) and direct OCI-registry HTTPS access — as MCP tools. It requires Python >=3.14 and is managed with `uv`. It is published to PyPI as **`docker-mcp-server`** and as a container image to GHCR (`ghcr.io/gavinlucas/docker-mcp-server`), mirrored to Docker Hub (`gavinlucas/docker-mcp-server`) when the opt-in `DOCKERHUB_*` release secrets are configured (the import package stays `docker_mcp`, the repo stays `…/docker-mcp`); two console scripts — `docker-mcp` and `docker-mcp-server` — both target `docker_mcp:main`. A third distribution channel packages the server as a **Claude Desktop Extension (`.mcpb`)** attached to each GitHub Release (see "Desktop Extension (MCPB)" below). A fourth channel is a **Homebrew tap** (`gavinlucas/homebrew-tap`) — see "Homebrew tap" below.
...
### Homebrew tap (`gavinlucas/homebrew-tap`)

A fourth distribution channel for macOS users who prefer Homebrew. `.github/workflows/publish-homebrew.yaml` runs on each GitHub Release: it exports the frozen lockfile deps with `uv export --frozen --no-dev --no-emit-project --no-hashes`, downloads pre-built binary wheels for `aarch64-apple-darwin` and `x86_64-apple-darwin` from PyPI using `uv pip download --only-binary :all:` (fails loudly if any dep lacks a macOS binary wheel — that is the guardrail), packages each set into a wheelhouse tarball, attaches both to the release, then clones `GavinLucas/homebrew-tap` and pushes an updated `Formula/docker-mcp-server.rb` (generated from a heredoc with version + sha256 substituted). Requires the repository secret `TAP_GITHUB_TOKEN` — a fine-grained PAT with Contents read+write on `GavinLucas/homebrew-tap`. `workflow_dispatch` accepts a `tag` input for backfilling. The formula creates a `uv` venv at `libexec`, installs `docker-mcp-server` from the offline wheelhouse (`--no-index --find-links=#{buildpath}`), and symlinks both console scripts into `bin/`. The formula file is always overwritten by the workflow — do not edit it in the tap repo by hand.
